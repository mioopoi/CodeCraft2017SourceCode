...
bool edge_used[MAX_EDGE_NUMBER];
...
public:
    ...
    void calculate_customer_in_flow();
    void calculate_edge_used();
    int node_num;
...
void NetSimplexMCF::init(int *servers, int level)
{
    ...
    this->init_edge_num = graph->edge_num;

	this->m = init_edge_num;
...
{
	int edge_num_origin = init_edge_num, cnt = 0;
...
void NetSimplexMCF::calculate_customer_in_flow()
{
    memset(simplex_flow_array, 0, sizeof(int) * node_num);
    memset(simplex_cost_array, 0, sizeof(int) * node_num);
    set_edge_flow();
    ...
    while(index != server_flow.size())
    {
        ...
        while(out_flow != 0)
        {
            ...
            {
                cur_flow = out_flow;
                simplex_flow_array[server_id] += cur_flow;
            }
            else
            {
                int des, cost = 0;
...
void NetSimplexMCF::calculate_edge_used()
{
    set_edge_flow();
