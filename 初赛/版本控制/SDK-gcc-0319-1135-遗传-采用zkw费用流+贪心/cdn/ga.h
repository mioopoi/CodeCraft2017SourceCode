...
public:
    ...
    void mutate();
    void keep_size();
    void add_an_Ind_from_out(vector<bool>& chromosome, int fitness);
private:
    int len_chromosome;
    int max_num_server;
...
/*write by 后锦*/
void GeneticAlgorithm::add_an_Ind_from_out(vector<bool>& chromosome, int fitness)
{
    Ind ind;
    ind.chromosome = chromosome;
    ind.feasible = true;
