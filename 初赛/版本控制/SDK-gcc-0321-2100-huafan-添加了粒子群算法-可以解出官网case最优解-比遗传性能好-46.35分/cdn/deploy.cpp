...
void test_PSO(Graph* graph, char* &solution)
{
    ParticleSwarmOpt* pso = new ParticleSwarmOpt(graph);
    pso->init_population();
    pso->pso_search();
    cout << "min cost found: " << pso->gbest_fitness << endl;
    // 构造输出
    pso->mcf->init(pso->gbest);
    pso->mcf->compute();
    vector<stack<int> > paths;
    pso->mcf->construct_solution(paths);
    construct_output(paths, solution);
}
