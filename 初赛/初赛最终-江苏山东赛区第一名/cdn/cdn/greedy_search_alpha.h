...
public:
    ...
    void construct_first_solution_by_node_detail();
    ...
    bool decrease_servers_by_servers_list();
    void construct_candidates_by_node_detail();

    int exchange_one_cnt;
    int decrease_cnt;
    int increase_cnt;
    int exchange_two_cnt;

    vector<vector<NeighborFlow>> neighbor_flows;
    void constuct_neighbor_flows();

    int customer_cost_pre[MAX_NODE_NUMBER];
    int customer_cost_now[MAX_NODE_NUMBER];
    void construct_customer_cost_now();
    bool increase_server_by_customer_cost();

    void test_increase_server_by_pre_flow();
...
GreedySearchAlpha::GreedySearchAlpha(Graph* graph)
{
    ...
    neighbor_flows = vector<vector<NeighborFlow>>(node_num, vector<NeighborFlow>(MAX_OUT_EDGE));

    memset(customer_cost_pre, -1, sizeof(customer_cost_pre));
    memset(customer_cost_now, 0, sizeof(customer_cost_now));

}

void GreedySearchAlpha::construct_customer_cost_now()
{
    memset(customer_cost_now, 0, sizeof(customer_cost_now));
    this->run_mcf(1);
    for(int i = 0; i < servers.size(); i++)
    {
        ...
        for(int j = 0; j < this->node_num; j++)
        {
            ...
            {
                customer_cost_now[j] +=  this->mcf_machine->cost_matrix[i][j];
...
bool GreedySearchAlpha::increase_server_by_customer_cost()
{
    this->construct_customer_cost_now();
    ...
    for(int i = 0; i < this->candidates.size(); i++)
    {
        ...
        if(customer_cost_pre[node_id] == customer_cost_now[node_id]) continue;
        is_feasilble = this->add_a_server(node_id);

        if(this->global_min_cost < tmp_cost)
        {
            //cout<<"succeed"<<endl;
            //cout<<customer_cost_pre[node_id]<<" "<<customer_cost_now[node_id]<<endl;
    ...
    this->construct_customer_cost_now();
    memcpy(customer_cost_pre, customer_cost_now, sizeof(customer_cost_pre));
    cout<<"total cost after increase_server_by_customer_cost: "<<global_min_cost<<endl;
...
{
    node_detail_list = vector<NodeDetail*>(node_num, NULL);
    ...
    for(int i = 0; i < node_num; i++)
    {
        ...
        //cout<<i<<" "<<degree<<" "<<max_bw<<" "<<cus_num<<endl;
...
void GreedySearchAlpha::construct_first_solution_by_node_detail()
{
    this->construct_candidates_by_node_detail();
    this->increase_servers_one();
}

void GreedySearchAlpha::construct_candidates_by_node_detail()
{
    this->init_neighbor_details();
    sort(node_detail_list.begin(), node_detail_list.end(), node_detail_cmp_greater());

    int num0 = int(this->node_num * 0);
    ...
    for(int i = 0; i < this->node_num; i++)
    {
        ...
        node_id = this->node_detail_list[i]->node_id;
...
void GreedySearchAlpha::topo_stratedy()
{
    greedy_degree = 0;
    //deal_topo_lv0_one();
    //this->construct_first_solution_by_node_detail();
    ...
    if(topo_lv == 0)
    {
       ...
       deal_topo_lv0_one();
        /*this->mcf_machine->init(this->servers);
       this->mcf_machine->run_zkw_mcf_record_all_cost();
       for(int i = 0; i < node_num; i++)
       {
            for(int j = 0; j < node_num; j++)
            {
                if(global_cost_matrix[i][j] != 0)
                    cout<<i<<" "<<j<<" "<<global_cost_matrix[i][j]<<" "<<endl;
            }
       }*/
       //deal_topo_lv1_zero();
    }
    ...
    this->run_mcf(1);
    cout<<"exchange one cnt: "<<exchange_one_cnt<<endl;
    cout<<"exchange two cnt: "<<exchange_two_cnt<<endl;
    cout<<"decrease cnt: "<<decrease_cnt<<endl;
    cout<<"increase cnt: "<<increase_cnt<<endl;
    //this->show_supply_detail();
    //this->run_mcf(1);
    //this->constuct_neighbor_flows();
    //construct_server_customer_costs(2);
}

void GreedySearchAlpha::test_increase_server_by_pre_flow()
{
    ...
    this->run_mcf();
    cout<<"increase cnt: "<<increase_cnt<<endl;
    cout<<"cicle cnt: "<<mcf_machine->cicle_cnt<<endl;

    /*cout<<"before increase: "<<mcf_machine->min_cost<<endl;
    cout<<mcf_machine->max_flow<<endl;
    //this->add_a_server(3);
    //cout<<"after increase: "<<mcf_machine->min_cost<<endl;
    mcf_machine->zkw_increase_one_server(servers, 3);*/
...
void GreedySearchAlpha::deal_topo_lv0_one()
{
    ...
    //decrease_servers_by_servers_list();
    //increase_servers_zero();
    //increase_server_by_customer_cost();

    /*construct_first_solution_by_node_detail();
    reverse(candidates.begin(), candidates.end());
    decrease_servers_by_servers_list();
    //decrease_servers_zero();*/
...
void GreedySearchAlpha::deal_topo_lv2_zero()
{
    /*this->construct_first_solution_zero();
    decrease_servers_zero();
    increase_servers_zero();*/
...
{
    /*this->servers[269] = false;
    this->servers[292] = false;
    this->servers[260] = true;
    //this->servers[77] = true;*/
...
bool GreedySearchAlpha::decrease_servers_by_servers_list()
{
    int ori_cost = global_min_cost;
    int node_id, tmp_cost = global_min_cost;
    bool is_feasilble;
    ...
    cout<<"total cost after decrease_servers_by_servers_list: "<<global_min_cost<<endl;
...
bool GreedySearchAlpha::increase_servers_zero()
{
    ...
    for(int i = 0; i < this->candidates.size(); i++)
    {
        ...
        this->run_mcf();
        this->mcf_machine->zkw_increase_one_server(servers, node_id);
        int tmp = mcf_machine->min_cost;
        is_feasilble = this->add_a_server(node_id);
        if(tmp != mcf_machine->min_cost)
        {
            cout<<"error: "<<endl;
        }
        //cout<<"real: "<<mcf_machine->min_cost<<endl;
        if(this->global_min_cost < tmp_cost)
        {
            //this->remove_server_after_increase(node_id);
            tmp_cost = global_min_cost;
...
void GreedySearchAlpha::constuct_neighbor_flows()
{
    mcf_machine->construct_neighbor_flows(servers, neighbor_flows);
}

bool GreedySearchAlpha::exchange_servers_one()
{
    this->run_mcf(1);
    constuct_neighbor_flows();
    this->construct_server_customer_costs(1);
    ...
    for(int i = node_num - 1; i >= 0; i--)
    {
        ...
        for(int j = 0; j < node_num; j++)
        {
            ...
            if(topo_lv != 3)
            {
                ...
                {
                    if(neighbor_flows[node_id][k].node_id == j)
                    {
                        if(neighbor_flows[node_id][k].flow != 0)
                            has_flow = true;
                        else
                            has_flow = false;
                        break;
                    }
                    //cout<<neighbor_flows[node_id][k].flow<<" "<<neighbor_flows[node_id][k].cost<<endl;
                }
            ...
            if(is_feasible && this->global_min_cost < tmp_cost)
            {
                ...
                //cout<<"succeed"<<" "<<node_id<<" "<<j<<endl;
                if(topo_lv != 2)    constuct_neighbor_flows();
                tmp_cost = this->global_min_cost;
